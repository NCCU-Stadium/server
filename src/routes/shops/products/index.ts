...
import {
    getAllProducts,
    getProductInfo,
} from '../../../database/shop/product/get';
import {
    updateProduct,
    updateProductInfo
} from '../../../database/shop/product/patch';
import {
    deleteProduct,
    deleteProductInfo,
} from '../../../database/shop/product/delete';
import {
    createProduct,
    addProductInfo,
} from '../../../database/shop/product/post';
...
router.post('/', jwtProtect, async (req, res) => {
    ...
    const { name, brand, price, desc, imgurl, size, color, sold, count } = req.body;
    try {
...
router.post('/:product_id', jwtProtect, async (req, res) => {
    ...
    try {
        const productStoreId = await addProductInfo(product_id, size, color, sold, count);
        res.status(200).json({ message: 'Success', productStore_id: productStoreId });
    } catch (err) {
        res.status(500).json({ message: err });
    }
});
...
router.delete('/:product_id/:product_store_id', jwtProtect, async (req, res) => {
    if (req.body.decoded.role != 'admin') {
        return res.status(500).send({ error: 'permission denied' })
    }
    ...
    try {
        ...
        res.status(200).json({ message: 'Delete a product information successfully' });
    } catch (err) {
        res.status(500).json({ message: err });
    }
});
